...
DEFAULT_GROUP_URL = "http://www.google.com/generate_204"
DEFAULT_GROUP_INTERVAL = 300


class Group(typing.TypedDict):
    name: str
    type: typing.Literal["select", "url-test", "fallback", "load-balance"]
    proxies: list[str]
    url: typing.NotRequired[str]
    interval: typing.NotRequired[int]
    strategy: typing.NotRequired[typing.Literal["round-robin", "consistent-hashing"]]


IPLC_NAME = "🎮IPLC_低延迟_5倍消耗"
PROXY_MARKER = "【代理】"

if __name__ == "__main__":
    with open(sys.argv[1], "r+", encoding="utf-8") as f:
        obj = yaml.safe_load(f)
        proxies = typing.cast(list[Proxy], obj["proxies"])
        proxy_groups = typing.cast(list[Group], obj["proxy-groups"])

        proxy_groups.append(
            Group(
                name=IPLC_NAME,
                type="select",
                proxies=[x["name"] for x in proxies if "IPLC" in x["name"]],
            )
        )
        ...
        for strat in ("round-robin", "consistent-hashing"):
            ...
            proxy_groups.insert(
                ...
                Group(
                    ...
                    type="load-balance",
                    strategy=strat,
                    url=DEFAULT_GROUP_URL,
                    interval=DEFAULT_GROUP_INTERVAL,
                    proxies=[x["name"] for x in proxies if "IPLC" not in x["name"]],
                ),
            )
            default_proxy["proxies"].append(name)

        all_proxygroup_names = set(
            x["name"] for x in proxy_groups if not x["name"].endswith("直接连接")
        )
        found_direct = False
        for group in proxy_groups:
            if not group["name"].endswith("直接连接"):
                group["name"] += PROXY_MARKER
            else:
                found_direct = True
            if "IPLC" not in group["name"]:
                group["proxies"] = [s for s in group["proxies"] if "IPLC" not in s]
            group["proxies"] = [
                (s + PROXY_MARKER if s in all_proxygroup_names else s)
                for s in group["proxies"]
            ]
        assert found_direct

        obj["rules"] = yaml.safe_load(
            """
  - PROCESS-NAME,Overwatch.exe,🎮IPLC_低延迟_5倍消耗
  - PROCESS-NAME,r5apex.exe,🎮IPLC_低延迟_5倍消耗
  - PROCESS-NAME,r5apex_dx12.exe,🎮IPLC_低延迟_5倍消耗

  - DOMAIN-SUFFIX,amtrak.com,🔰国外流量
  - DOMAIN-SUFFIX,cloudfunctions.net,🔰国外流量
  - DOMAIN-SUFFIX,metacareers.com,🔰国外流量
  - DOMAIN-SUFFIX,goo.gl,🔰国外流量
  - DOMAIN-SUFFIX,digwebinterface.com,🔰国外流量

  # ===== no need to proxy steam chat, Steam => Settings => Web Browser => Clear data/cache =====
  # - DOMAIN-SUFFIX,steamserver.net,🔰国外流量
  # - DOMAIN-SUFFIX,steam-chat.com,🔰国外流量

  - DOMAIN-SUFFIX,entrata.com,🔰国外流量
  - DOMAIN,bitwarden.com,🔰国外流量
  - DOMAIN,linkedin.com,🔰国外流量

  # ===== Otherwise will be directed to *.cn =====
  - DOMAIN-SUFFIX,leetcode.com,🔰国外流量
  - DOMAIN,www.linkedin.com,🔰国外流量
  - DOMAIN,linkedin.com,🔰国外流量
  - DOMAIN,platform.linkedin.com,🔰国外流量

  - DOMAIN-SUFFIX,stackpath.bootstrapcdn.com,🔰国外流量
  ...
  - DOMAIN-SUFFIX,freevod.org,🔰国外流量
  ...
  - DOMAIN-SUFFIX,de,🔰国外流量
  - DOMAIN-SUFFIX,crocs.com,🔰国外流量
  - DOMAIN-SUFFIX,vercel.app,🔰国外流量
  - DOMAIN-SUFFIX,github.githubassets.com,🔰国外流量
  - DOMAIN-SUFFIX,diskanalyzer.com,🔰国外流量
  - DOMAIN-SUFFIX,webpkgcache.com,🔰国外流量
  - DOMAIN-SUFFIX,googleapis.com,🔰国外流量
  - DOMAIN-SUFFIX,googleusercontent.com,🔰国外流量
  - DOMAIN-SUFFIX,us.battle.net,🔰国外流量
  - DOMAIN-SUFFIX,hostloc.com,🔰国外流量
  - DOMAIN-SUFFIX,cn.bing.com,DIRECT
  - DOMAIN,gist.github.com,🔰国外流量
  - DOMAIN,github.com,🔰国外流量
  - DOMAIN-SUFFIX,github.blog,🔰国外流量
  - DOMAIN-SUFFIX,gravatar.com,🔰国外流量
  - DOMAIN-SUFFIX,v2ex.com,🔰国外流量
  - DOMAIN-SUFFIX,smtp,DIRECT
  - DOMAIN-KEYWORD,aria2,DIRECT
  - DOMAIN-SUFFIX,ipv6check-http.steamserver.net,🔰国外流量
  ...
  - DOMAIN-SUFFIX,beta.openai.com,💬chatGPT
  - DOMAIN-SUFFIX,auth0.openai.com,💬chatGPT
  - DOMAIN-SUFFIX,auth1.openai.com,💬chatGPT
  - DOMAIN-SUFFIX,auth2.openai.com,💬chatGPT
  ...
  - DOMAIN-SUFFIX,playback-certs.bamgrid.com,🎬Disney+
  - DOMAIN-SUFFIX,disney.api.edge.bamgrid.com,🎬Disney+
  - DOMAIN-SUFFIX,disney.connections.edge.bamgrid.com,🎬Disney+
  ...
  - DOMAIN-SUFFIX,sanalytics.disnyplus.com,🎬Disney+
  ...
  - DOMAIN-SUFFIX,google.com.hk,🔰国外流量
...
"""
        ...
        proxies_name = set(x["name"] for x in proxy_groups)
        for i in range(len(obj["rules"])):
            s = typing.cast(str, obj["rules"][i])
            if not s.endswith(",DIRECT") and not s.endswith(",DIRECT,no-resolve"):
                split_s = s.split(",")
                group_name_idx = -1
                if split_s[-1] == "no-resolve":
                    group_name_idx = -2
                split_s[group_name_idx] += PROXY_MARKER
                assert (
                    split_s[group_name_idx] in proxies_name
                ), f'Proxy group {s.split(",")} not found!'
                obj["rules"][i] = ",".join(split_s)
